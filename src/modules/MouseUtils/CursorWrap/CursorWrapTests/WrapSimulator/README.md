...
A Python visualization tool that displays monitor layouts and shows which edges will wrap to other monitors using the retained upstream CursorWrap test implementation.

> Note: CursorWrap is not a MagicalGirlWand product feature. This tool remains in the repository because parts of the upstream dependency graph are still retained while the standalone CmdPal graph is being trimmed.
...
## Integration with Retained CursorWrap Tests
