...
- 把本仓库视为独立 CmdPal 项目，不是完整 Microsoft PowerToys。
- 只写当前仓库真实存在的功能。
...
- 人名、账号、ID、项目名、库名和工具名保持原文，不要翻译或改写。
...
- 行为变化要添加或更新测试。
- hot path 不加噪声日志。
- 保持 `tools/cmdpal/standalone-keep-roots.txt` 和真实依赖图一致。
...
- 独立 CmdPal 范围变化：`pwsh -NoProfile -File tools\cmdpal\Verify-CmdPalStandalone.ps1`。
